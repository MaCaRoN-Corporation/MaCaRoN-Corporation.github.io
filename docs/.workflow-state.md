...
- [x] **Step 9:** SM - Create stories (repeats for each epic)
  - Status: ✅ Completed for Epic 1, 2, 3
  - Epic 1 Stories: ✅ All 6 stories created and validated
  - Epic 2 Stories: ✅ All 9 stories created and validated
  - Epic 3 Stories: ✅ All 8 stories created and validated
  - Locations:
...
- [x] **Step 11:** QA - Review implementations
  ...
  - Status: ✅ **COMPLÈTE** pour Epic 4 (100% - 6/6 stories)
  - Story 4.1: ✅ Done (Audio Service with Local Audio Files - implémenté avec logique hiérarchique)
  - Story 4.3: ✅ Done (Audio Playback During Passage - intégré dans PassageComponent)
  - Story 4.4: ✅ Done (Pause and Resume Controls - UI et raccourci Espace implémentés dans passage.ts)
  - Story 4.5: ✅ Done (Repeat Last Technique Control - clic sur glassmorphism implémenté dans PassageComponent)
  - Story 4.6: ✅ Fait (Fullscreen Mode Implementation - implémenté dans navigation.ts)
  - Story 4.7: ✅ Done (Keyboard Shortcuts - Entrée, Flèches implémentés dans passage.ts et home.ts)
...
- ✅ `src/app/services/audio.service.ts` - AudioService complètement implémenté (Story 4.1)
- ✅ `src/app/services/audio.service.spec.ts` - Tests unitaires AudioService (Story 4.1)
- ✅ `docs/stories/4.1.audio-service-with-local-audio-files.md` - Story 4.1 complétée
- ✅ `docs/stories/4.3.audio-playback-during-passage.md` - Story 4.3 complétée
...
- Status: ✅ **COMPLÈTE**
- Total Stories: 6 (Story 4.2 ElevenLabs supprimée)
- Completed Stories: 6/6 (100%)
- Stories Status:
  - Story 4.1: ✅ Done (Audio Service with Local Audio Files)
    ...
    - ✅ Normalisation des noms de fichiers (minuscules + underscores)
    - ✅ Support armes et randori
    - ✅ Tests unitaires en place
  - Story 4.2: ❌ Supprimée (ElevenLabs non prévu - audios locaux uniquement)
  - Story 4.3: ✅ Done (Audio Playback During Passage)
    - ✅ AudioService intégré dans PassageComponent
    - ✅ `resetComparisonState()` appelé au début d'un nouveau passage
    - ✅ `playTechnique()` appelé automatiquement lors des changements de technique
    - ✅ Audio joue immédiatement quand la technique change
    - ✅ Intégration pause/resume avec les contrôles du passage
    - ✅ Arrêt et nettoyage de l'audio dans ngOnDestroy
    - ✅ Gestion d'erreurs gracieuse
  - Story 4.4: ✅ Done (Pause and Resume Controls)
  ...
  - Story 4.5: ✅ Done (Repeat Last Technique Control)
    - ✅ Clic sur le glassmorphism de la technique annoncée pour répéter (implémenté dans `passage.html`)
    - ✅ Méthode `repeatLastTechnique()` implémentée dans `passage.ts`
    - ✅ Intégration avec `audioService.repeatLastTechnique()`
    - ✅ Section technique désactivée (opacité réduite) si aucune technique (`[class.disabled]="!currentTechnique"`)
    - ✅ Fonctionne même si le passage est en pause
    - ✅ Ne perturbe pas l'état du passage (isPaused reste inchangé)
    - ✅ Style cursor pointer avec feedback visuel (hover/active) et tooltip
  - Story 4.6: ✅ Fait (Fullscreen Mode Implementation - implémenté dans `navigation.ts`)
  - Story 4.7: ✅ Done (Keyboard Shortcuts)
    ...
    - ✅ Raccourci Espace déjà fait dans Story 4.4

**Note Epic 4:** 
- Les fichiers audio sont disponibles dans `src/assets/audio/` (501 fichiers MP3)
- ✅ AudioService complètement implémenté et fonctionnel
- ✅ Audio intégré dans le passage (lecture automatique des techniques)
- ✅ Bouton pause/resume implémenté dans l'UI avec raccourci clavier Espace fonctionnel
- ✅ Clic sur glassmorphism de la technique pour répéter (Story 4.5)
- ✅ Le mode plein écran est déjà implémenté dans la bannière (`navigation.ts`)
- ✅ Tous les raccourcis clavier implémentés (Story 4.7) :
  - Page passage : Espace (pause/resume), Entrée (répéter), Flèche droite (skip)
  - Page accueil : Flèches gauche/droite (carrousel type de passage)

**Epic 5: Personalization & Export**
- Status: ⚠️ **PARTIELLEMENT FAIT** (60% - 3/5 stories)
- Story 5.1: ✅ COMPLÈTE (Settings Page Layout - page créée, accessible, responsive)
- Story 5.2: ⚠️ Partiellement fait (Color Customization - infrastructure en place, color pickers UI manquants)
- Story 5.3: ✅ COMPLÈTE (Theme Persistence - déjà fait dans Story 1.5 et 1.6, 9 palettes de couleurs)
- Story 5.4: ⚠️ Partiellement fait (Export Service - service créé mais méthodes vides)
- Story 5.5: ⚠️ Partiellement fait (Export UI - bouton présent dans passage.html, fonctionnalité manquante)

**Epic 6: History & Final Polish**
- Status: ⚠️ **PARTIELLEMENT FAIT** (12.5% - 1/8 stories)
- Story 6.1: ⏳ À faire (History Storage Service - à créer)
- Story 6.2: ⚠️ Partiellement fait (History Page - page créée mais vide, placeholder seulement)
- Story 6.3: ⚠️ Partiellement fait (Animations - compte à rebours, progression, confetti existent, transitions manquantes)
- Story 6.4: ⚠️ Partiellement fait (Responsive - déjà bien implémenté, optimisations supplémentaires possibles)
- Story 6.5: ⏳ À faire (Orientation Detection - à implémenter)
- Story 6.6: ⚠️ Partiellement fait (Performance - optimisations de base, lazy loading à considérer)
- Story 6.7: ⚠️ Partiellement fait (Error Handling - gestion de base, messages à améliorer)
- Story 6.8: ⚠️ Partiellement fait (UI/UX Polish - UI déjà polie, vérification finale contraste WCAG)

**Next Action:** Compléter les stories partiellement faites de l'Epic 5 (color pickers, export) puis implémenter l'Epic 6 (historique, optimisations)
